...
namespace EcommerceUtilities
{
    public class PayPalFunctions
    {
        ...
        public string APIUsername = "zia_7-facilitator_api1.outlook.com";
        private string APIPassword = "1406396257";
        private string APISignature = "AFcWxV21C7fd0v3bYYYRCpSSRl31AWTkrf8oTSsoYoV3XqSEnU-.why2";
        ...
        public bool ShortcutExpressCheckout(string amt, ref string token, ref string retMsg)
        {
            ...
            string returnURL = "http://localhost:40411/Checkout/CheckoutReview.aspx";
            string cancelURL = "http://localhost:40411/Checkout/CheckoutCancel.aspx";

            NVPCodec encoder = new NVPCodec();
            encoder["METHOD"] = "SetExpressCheckout";
            encoder["RETURNURL"] = returnURL;
            encoder["CANCELURL"] = cancelURL;
            encoder["BRANDNAME"] = "Ecommerce Application";
            ...
            if (_allCartItems != null)
            {
                ...
                var allProducts = HttpContext.Current.Session["CartItems"] as List<tbl_Products>;
                if (allProducts != null)
                    foreach (var item in allProducts)
                    {
                        encoder["L_PAYMENTREQUEST_0_NAME" + i] = item.ProductName;
                        //string x = ProductCommulativePricePayPal(item.ProductID.ToString(), QuantityOfProductInCartItemsPaypal(item.ProductID.ToString()), item.ProductUnitPrice);
                        int amt1 = Convert.ToInt32(item.ProductUnitPrice);
                        encoder["L_PAYMENTREQUEST_0_AMT" + i] = amt1.ToString();
                        string y = QuantityOfProductInCartItemsPaypal(item.ProductID.ToString());
                        encoder["L_PAYMENTREQUEST_0_QTY" + i] = y;
                        i++;
                    }
            }
        ...
        /// <summary>
        /// Code by zia : return accumulated price of the product to send to paypal
        /// </summary>
        /// <param name="pId"></param>
        /// <param name="count"></param>
        /// <param name="unitPrice"></param>
        /// <returns></returns>
        public string ProductCommulativePricePayPal(string pId, string count,decimal unitPrice)
        {
            int quantity;
            long prodId;
        ...
        protected string QuantityOfProductInCartItemsPaypal(string productId)
        {
            long prodId;
            int quantity;
