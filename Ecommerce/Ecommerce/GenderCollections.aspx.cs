...
namespace Ecommerce
{
    ...
    public partial class GenderCollections : FrontBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
               if (Request.QueryString["GId"] != null && int.TryParse(Request.QueryString["GId"],out _gId))
               {
                   PopulateAllProducts();
               }
               else
        ...
        private void PopulateAllProducts()
        {
             using (var clothEntities = new ClothEntities())
             {
                 ...
                 var tblProducts = clothEntities.SP_ProductSearchFront(string.Empty, -1, -1,_gId, -1, 1, -1, -1,
                                                                    StoreId, pageIndex,pageSize, string.Empty, string.Empty,totalRowsCount).ToList();
        ...
        private void PopulateValuesViaViewState()
        {
           ...
           if (CheckViewState(GId))
               _gId = int.Parse(ViewState[GId].ToString());
